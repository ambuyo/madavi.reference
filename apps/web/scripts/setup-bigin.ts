...
if (!REFRESH_TOKEN || !CLIENT_ID || !CLIENT_SECRET) {
  console.error(
    "❌  Missing ZOHO_REFRESH_TOKEN, ZOHO_CLIENT_ID, or ZOHO_CLIENT_SECRET in .env",
  );
  process.exit(1);
}
...
const CONTACT_FIELDS: FieldDef[] = [
  ...
  },
  {
    ...
    type: "picklist",
    values: ["Not started", "Exploring", "Piloting", "Scaling", "Optimizing"],
  },
  {
  ...
  },
  {
    ...
    type: "picklist",
    values: ["$0", "Less than $500", "$500–$2K", "$2K–$10K", "$10K+"],
  },
  {
    label: "Dedicated AI Team",
    type: "picklist",
    values: [
      "Full dedicated team",
      "Part-time resources",
  ...
  { label: "Team AI Literacy", type: "integer" },
  {
    ...
    type: "picklist",
    values: ["Strong champion", "Supportive", "Neutral", "Skeptical"],
  },
  {
...
async function createField(
  token: string,
  f: FieldDef,
): Promise<{ label: string; apiName: string }> {
...
async function main() {
  ...
  // ── Custom fields ──────────────────────────────────────────────────────────
  console.log(
    `📋  Creating ${CONTACT_FIELDS.length} custom fields on Contacts...\n`,
  );
  ...
  console.log(
    pipelineId
      ...
      : "    ↩  Already exists or check manually",
  );
  ...
  fs.writeFileSync(
    outputPath,
    JSON.stringify({ fields: fieldMap, pipelineId }, null, 2),
  );
