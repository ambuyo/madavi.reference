...
export const onRequest = defineMiddleware((context, next) => {
  ...
  if (categoryMatch) {
    return Response.redirect(
      new URL(`/blog/cat/${categoryMatch[1]}`, url.origin).toString(),
      301,
    );
  }
  ...
  if (writerMatch) {
    return Response.redirect(
      new URL(`/blog/author/${writerMatch[1]}`, url.origin).toString(),
      301,
    );
  }
  ...
  if (workMatch) {
    return Response.redirect(
      new URL(`/our-work/${workMatch[1]}`, url.origin).toString(),
      301,
    );
  }
