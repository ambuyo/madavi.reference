...
export const BreakableCard = ({
  ...
  const [cardId] = useState(() => `card-${crypto.randomUUID().slice(0, 9)}`);
  const [figNum] = useState(() => Math.floor(Math.random() * 99) + 1);
    ...
    <div className={cn("relative w-full h-full", className)}>
      ...
      >
        ...
        <div className="mt-8 border-t-2 border-black pt-2 flex justify-between text-xs font-mono uppercase">
          <span>
            {subtitle ? title : `Fig. ${figNum.toString().padStart(2, "0")}`}
          </span>
